    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            // var mine = Mine(location: location)
             var mine = Mine(location: GridLocation(x:location.x,y:location.y), guaranteesHit: false, penaltyText: "Boom! it's a Mine")
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
               // mine = Mine(location: location)
                mine = Mine(location: GridLocation(x:location.x,y:location.y),  guaranteesHit: false, penaltyText: "Boom! it's a Mine")
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            ...
            var seaMonster = SeaMonster(location: GridLocation(x:location.x,y:location.y), guaranteesHit: true, penaltyText: "It's Sea Monster")
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                // seaMonster = SeaMonster(location: location)
                seaMonster = SeaMonster(location: GridLocation(x:location.x,y:location.y), guaranteesHit: true, penaltyText: "It's Sea Monster")
            }
        }
    }
}